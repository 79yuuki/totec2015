...
// Connection URL
var local = 'mongodb://localhost:27017/totec';
var server = 'mongodb://172.30.2.36:27017/totec';
 ...
 * @param {number} limit 上限指定
 * @param callback コールバック関数
 * http://docs.mongodb.org/manual/reference/method/db.collection.find/
 */
function find(collection_name, criteria, projection, limit, callback){
    if (limit typeof === "function") {
      callback = limit;
    }
    db.collection(collection_name, function(outer_error, collection){
        if (limit) {
          collection.find(criteria, projection).limit(Number(limit)).toArray(function(inner_error, list){
              callback(list);
          });
        } else {
          collection.find(criteria, projection).limit(100).toArray(function(inner_error, list){
              callback(list);
          });
        }
    });
